...
def get_food():
    ...
    # Get recommendation
    recomendation = get_recomendation(
...
#     bmi = profile['bmi']
#     diet = mongo.db.diet_prefs.find_one({'_id': profile['dietPrefId']})
...
#     calories = "{:.1f}".format((float(bmr) * 1.2) / 9)

#     carbs = "{:.1f}".format((float(calories) * 45/100) / 7)

#     fat = ((1.20 * bmi) + (0.23 * age) - 5.4) / 7

#     protein = "{:.1f}".format((float(weight) * 0.8) / 7)

#     types = 1 if diet['name'] != "non vegan" else 0
